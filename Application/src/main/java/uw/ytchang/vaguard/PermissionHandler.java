...
public class PermissionHandler {
    ...
    public final static int INTERNET = 2;
    ...
    public static void askForPermission(int which, final Activity activity) {
        ...
        } else //We are running on Android M
        {
            switch (which) {
                ...
                case INTERNET:
                    if (ContextCompat.checkSelfPermission(activity, Manifest.permission.INTERNET) == PackageManager.PERMISSION_GRANTED && ContextCompat.checkSelfPermission(activity, Manifest.permission.GET_ACCOUNTS) == PackageManager.PERMISSION_GRANTED)
                        return;
                    else {
                        if (ActivityCompat.shouldShowRequestPermissionRationale(activity, Manifest.permission.INTERNET)) {
                            Toast.makeText(activity, activity.getString(R.string.internet_require), Toast.LENGTH_LONG).show();
                        } else {
                            ActivityCompat.requestPermissions(activity, new String[]{Manifest.permission.INTERNET}, INTERNET);
                        }
                    }
                    break;
            }
